...
function buildRegion(G, rx, rz) {
  ...
  /* A deck is laid level with the higher bank. It used to be lifted half a
     metre over the water level this pass reads, but that is the field's
     smooth river level, before the window drains what its banks cannot hold
     — it stood 0.75 m over a bank that the water in the built world sits far
     below, and the deck hung in the air at both ends. Standing water never
     tops its lower bank, so the higher one always clears it. */
  function deckHeight(a, b) { return Math.max(a.H, b.H); }
  ...
  function markPath(path, road) {
    ...
    while (w < path.length) {
      ...
      if (len <= 12) {
        ...
        if (d) {
          /* The deck's line is trail, landings included: it need not be the
             route's own cells, and nothing but the trail keeps a boulder off
             a landing or levels one against the bank it meets. */
          var si = Math.min(d.ai, d.bi), sj = Math.min(d.aj, d.bj), ui = d.bi - d.ai ? 1 : 0, uj = ui ? 0 : 1;
          for (var t = -1; t <= d.len + 1; t++) {
            var mi = si + ui * t, mj = sj + uj * t;
  ...
  /* The straight, axis-aligned deck that carries a route over the water it
     crosses: dry bank to dry bank, within a step of each other, and landing
     as near the route's own banks as it can.

     It used to be laid between the route's last dry cell and its next, on
     whichever axis it had moved in, taking x if both. A route is not a
     straight line: one that crossed 6 m in z and drifted 1 m in x laid its
     deck along the river with both ends in it, and one that crossed on a
     diagonal clipped the water at an end. So every wet cell of the crossing
     is tried as a place to lay a deck, on both axes, and the shortest that
     lands on dry ground at both ends wins. None does — deep water with no
     bank a step apart — and there is no bridge; the route wades. */
  function deckOver(path, st, w, pre, post) {
    var best = null, q, ax;
    var pi = (pre / N) | 0, pj = pre % N, qi = (post / N) | 0, qj = post % N;
    for (q = st; q < w; q++) {
      var ci = (path[q] / N) | 0, cj = path[q] % N;
      for (ax = 0; ax < 2; ax++) {
        var di = ax ? 0 : 1, dj = ax ? 1 : 0, ai = ci, aj = cj, bi = ci, bj = cj, n;
        for (n = 0; n < 14 && at(ai, aj).water; n++) { ai -= di; aj -= dj; if (ai < PAD || aj < PAD) break; }
        ...
        /* How far the deck's ends are from where the route meets it, either
           way round: a deck the trail cannot find is no crossing. */
        var off = Math.min(Math.abs(ai - pi) + Math.abs(aj - pj) + Math.abs(bi - qi) + Math.abs(bj - qj),
                           Math.abs(ai - qi) + Math.abs(aj - qj) + Math.abs(bi - pi) + Math.abs(bj - pj));
        var score = span + off * 2;
        if (!best || score < best.score) {
          var fwd = Math.abs(ai - pi) + Math.abs(aj - pj) <= Math.abs(bi - pi) + Math.abs(bj - pj);
          best = { score: score, ai: ai, aj: aj, bi: bi, bj: bj, a: A, b: B, len: span,
                   di: fwd ? di : -di, dj: fwd ? dj : -dj };
        }
      }
    }
    return best;
