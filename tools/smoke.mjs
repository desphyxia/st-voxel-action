 ...
 *   node tools/smoke.mjs --browser --only=build,magma   # only those browser groups
 *   node tools/smoke.mjs --browser --affected[=<base>]  # only what the diff can reach
 *   node tools/smoke.mjs --list          # the browser groups, and what each covers
 *
 * The browser half is seven groups — boot, render, build, stream, worker,
 * magma, look — and each run ends with how long each took. Locally, run the
 * ones a change can reach and let CI, which runs them all, be the full gate:
 * tools/lib/affected.mjs maps changed paths onto groups, conservatively.
 *
...
import { GROUPS, affected } from './lib/affected.mjs';
...
/* Which browser groups to run — see tools/lib/affected.mjs. --only names them,
   --affected picks them from what changed since a base, and neither runs all. */
const flag = (name) => { const a = argv.find((x) => x === name || x.startsWith(name + '=')); return a === undefined ? null : a.slice(name.length + 1); };
if (argv.includes('--list')) {
  for (const [g, what] of Object.entries(GROUPS)) console.log(`  ${g.padEnd(7)} ${what}`);
  process.exit(0);
}
let WANT = Object.keys(GROUPS);
if (flag('--only') !== null) {
  WANT = flag('--only').split(',').map((g) => g.trim()).filter(Boolean);
  const bad = WANT.filter((g) => !GROUPS[g]);
  if (bad.length || !WANT.length) {
    console.error(`unknown browser group ${bad.join(', ') || '(none given)'} — node tools/smoke.mjs --list`);
    process.exit(2);
  }
} else if (flag('--affected') !== null) {
  const a = affected(ROOT, flag('--affected') || 'origin/main');
  console.log(`changed since ${a.base}:`);
  for (const [path, gs] of a.why) console.log(`  ${path.padEnd(36)} ${gs.length ? gs.join(' ') : '—'}`);
  WANT = a.groups;
  console.log(`browser groups: ${WANT.length ? WANT.join(' ') : 'none'}\n`);
}
const runs = (g) => WANT.includes(g);
const BROWSER_HALF = !argv.includes('--node') && !UPDATE && WANT.length > 0;
...
/* How long each browser group took, printed at the end: the half is slow
   enough that knowing where the time went is what makes --only worth using. */
const laps = [];
let lapName = null, lapAt = Date.now();
const lap = (g) => {
  const now = Date.now();
  if (lapName) laps.push([lapName, now - lapAt]);
  lapName = g; lapAt = now;
};
...
if (BROWSER_HALF) {
  ...
  try {
    ...
    if (!QUICK && !UPDATE) {
      ...
      /* Every group below opens the build from here. */
    ...
    lap(null);
    await browser.close();
    if (laps.length) console.log('\nbrowser groups: ' + laps.map(([g, ms]) => `${g} ${(ms / 1000).toFixed(0)} s`).join(', '));
  }
}
...
const partial = !argv.includes('--node') && !UPDATE && WANT.length < Object.keys(GROUPS).length;
console.log(`\nall checks passed${partial ? ` — browser groups ${WANT.join(', ') || 'none'} only; CI runs the rest` : ''}`);
